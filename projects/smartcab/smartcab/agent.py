...
class LearningAgent(Agent):
    ...
    def build_Q(self):
        """ Brute force construct all possible states"""
    ...
    def get_max_action(self, state):
        """ The get_max_action function is called when the agent is asked to find the
            action associated with maximum Q-value of all actions based on the
            'state' the smartcab is in. """
    ...
    def get_zero_action(self, state):
        """ The get_mid_action function is called when the agent is asked to find the
            action associated with an unchecked (zero valued) Q-value of all actions
    ...
    def learn(self, state, action, reward):
        ...
        # score = (1.0 - self.alpha) * self.Q[state][action] + self.alpha * (reward + self.get_maxQ(state))
...
def run():
    ...
    sim = Simulator(env, update_delay=2.0, display=True, log_metrics=True, optimized=True)
